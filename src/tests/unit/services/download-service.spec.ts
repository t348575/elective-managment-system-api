...
import { mockCreateReadStream, mockCreateWriteStream, MockFsFilestream } from '../../mocks/mock-fs-filestream';
...
describe('Downloads service', () => {
    ...
    it('Should create temporary download link', async () => {
        const res = await downloadService.addTemporaryUserLink(
            [users[56]].map((e) => e.id as string),
            '/to/some/path/some_file.txt',
            'some_file.txt'
        );
    ...
    it('Should get class file', async () => {
        const fileId = await downloadService.addTemporaryUserLink(
            [users[56]].map((e) => e.id as string),
            '/to/some/path/some_file.txt',
            'some_file.txt'
        );
        MockFsFilestream.createWriteStream('/to/some/path/some_file.txt', { flags: '' }).write(
            Buffer.from('some_random_data')
        );
        const resObj = {};
    ...
    it('Should get temporary file', async () => {
        ...
        const resObj = {};
        await downloadService.getClassResource(
            fileId,
            (classes[0].students[0] as never) as string,
            resObj as express.Response
        );
        expect(resObj).to.have.property('body');
        // @ts-ignore
        expect(resObj.body.toString()).to.equal('some_random_data');
